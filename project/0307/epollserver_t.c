...
void write_message(int fd, struct user *user_list,  int *current_users, char *buf);
...
//                printf(" %s connected client : %d \n", nickname, client_socket);
                ...
                if (str_len == 0) {
                    ...
                    exit_user(ep_events[i].data.fd, user_list,  &current_users);
//                    show_users(user_list);
                    printf("현재 유저 수 %d \n", current_users);
                    ...
                    printf("메시지 전송 요청");
//                    print_set(&user_num);
                    write_message(ep_events[i].data.fd, user_list, &current_users, buf);
                }
            }
        }
    }
